...
代码调用的是 OpenAI-compatible Chat Completions 接口：

```text
POST {AI_API_BASE_URL}/chat/completions
```

本地 `.env` 当前已配置为小米 MiMo：
...
1. 打开首页。
2. 输入项目标题。
3. 上传 `.txt` 小说文件，或直接粘贴小说文本。
4. 点击“上传并生成”。
5. 进入“分析”页查看章节、人物、地点。
6. 进入“预览”页查看场景卡片和剧本文本。
7. 在“预览”页选择场景，点击“强化冲突”或“修改对白”。
8. 进入“导出”页下载 YAML 或 Markdown。
...
```powershell
python -c "from backend.services.ai_client import LLMSettings; s=LLMSettings.from_env(); print({'has_key': bool(s.api_key), 'base_url': s.api_base_url, 'model': s.model, 'mock_mode': s.mock_mode})"
```
...
说明已经调用到小米 MiMo 服务，但账户余额不足。需要在小米 MiMo 控制台充值或换可用 key。
